...
.PARAMETER Passthru
    ...
    NOTE: Content type must be text/html, text/xml, Content
...
function Get-WebFile {
	...
	try {
		...
		Write-Verbose "Responce Status: $($response.StatusCode) ContentType: $($response.ContentType) CharacterSet: $($response.CharacterSet)"
   	} catch {
    ...
    try {
        ...
        $outDir = (Get-Location -PSProvider "FileSystem")
        if (Test-Path -PathType "Container" $OutFile) {
            $outDir = $OutFile
            $OutFile = $null
        }
        ...
        if ( ($response.StatusCode -eq 200) `
            ...
            try {
                ...
                do {
                    ...
                    $total += $count
                    if ($goal -gt 0) {
                        Write-Progress "Downloading $Url" "Saving $total of $goal bytes..." `
                            -id 0 -PercentComplete (($total/$goal)*100)
                    } else {
                        Write-Progress "Downloading $Url" "Saving $total bytes..." `
                            -id 0
                    }
                } while ($count -gt 0)
            } catch {
